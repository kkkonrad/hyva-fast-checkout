...
- **Required Shipping Fields**: Tabela pozwalająca wybrać metodę dostawy i zaznaczyć wymagane pola z listy. Dla integracji niestandardowych można dopisać dokładne ścieżki pól rozdzielone przecinkami. Reguły dostawy mogą używać wildcardów typu `carrier_*`.
- **Required Payment Fields**: Tabela pozwalająca wybrać metodę płatności i zaznaczyć wymagane pola z listy. Dla integracji niestandardowych można dopisać dokładne ścieżki pól; kod metody płatności jest zawsze dopasowywany dokładnie.
...
- **Domyślne integracje pól wymaganych**: Konfiguracja zawiera `purchaseorder.po_number` oraz `instore_pickup.extension_attributes.pickup_location_code`, zgodnie z kontraktami Magento Offline Payments i Magento Inventory In-Store Pickup.
