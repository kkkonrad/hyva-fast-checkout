...
- **Required Shipping Fields**: JSON określający wymagane pola dla metod dostawy, np. `{"customcarrier_pickup":["custom_attributes.pickup_location_code"],"inpost_*":["extension_attributes.locker_id"]}`. Reguły dostawy mogą używać wildcardów typu `carrier_*`.
- **Required Payment Fields**: JSON określający wymagane pola dla wybranych metod płatności, np. `{"purchaseorder":["po_number"],"custom_gateway":["additional_data.transaction_id"]}`. Kody metod płatności używają wyłącznie Exact match.
