/**
 * Fastcheckout shipping-method presentation on top of Magento_Checkout/js/view/shipping.
 *
 ...
 *    carriers, so the veil is noise and it used to swallow clicks);
 *  - Tailwind css/price helpers used by Kkkonrad_Fastcheckout/hyva/shipping-list;
 *  - a per-method + list-level error model (richer than core's single errorValidationMessage);
 *  - the optional InPost widget hook.
 *
 * The same single core instance renders in two places: the address block
 * (Kkkonrad_Fastcheckout/hyva/shipping-address) and the shipping-method block
 * (Kkkonrad_Fastcheckout/hyva/shipping-list). A second instance is NOT an option — core's
 * initialize() registers a step navigator entry, resolves the shipping address and calls
 * shippingRatesValidator.initFields() with a hardcoded fieldset name.
 */
define([
    'ko',
    ...
    if (!window.fastcheckoutShippingClickCaptureBound) {
        ...
        }, true); // click only — change is ignored (see above) to prevent double apply
    }
    ...
    return function (Shipping) {
        return Shipping.extend({
            ...
            initObservable: function () {
                ...
                // Core declares errorValidationMessage as a plain ko.observable(false) and
                // clears it on quote.shippingMethod change. Fastcheckout needs a per-method
                // code alongside the message, so both are tracked here; core's auto-clear
                // still works and simply blanks the message.
                this.observe({
                    errorMethodCode: '',
                ...
                // Shadows core's `isLoading: shippingService.isLoading`. Picking a rate never
                // re-estimates carriers, so the list must not be veiled while it happens.
                // Magento's shipping-rates-validator still raises isLoading(true) on the
                // address re-notify that follows a selection, and that estimate resolves ~2s
                // later — long after the locks drop — which read as "the list reloads when I
                // pick a method". Address-driven estimates keep their spinner: that one is
                // real feedback. shippingService.isLoading itself is left untouched so the
                // payment area and other consumers behave as before.
                ...
                this.selectedMethodCode = ko.pureComputed({
                    read: function () {
                        ...
                        if (!active) {
                            checkedDomRadio = document.querySelector('input[name="shipping_method"]:checked');
                    ...
                    write: function (value) {
                        ...
                        if (typeof self.clearError === 'function') {
                            self.clearError();
                        }
                        rates = shippingService.getShippingRates()();
                        rates.some(function (rate) {
                            var c1 = rate.carrier_code + '_' + rate.method_code,
                                c2 = rate.method_code + '_' + rate.carrier_code;
                        ...
                        if (!found) {
                            parsed = self.splitMethodCode(value);
                            found = {
            ...
            hasError: function (method) {
                var self = this.errorMethodCode ? this : (window.fastcheckoutHyvaShippingListInstance || this),
                    ...
                    err = typeof self.errorMethodCode === 'function' ? self.errorMethodCode() : '';
